...
public class BeautyViewEx extends BaseView
{
	...
	protected void onDraw(Canvas canvas)
	{
		...
		//画图片
		if(isDrawImg() && img != null && img.m_bmp != null && !img.m_bmp.isRecycled())
		{
			canvas.save();
	...
	protected void KickBack(Shape item)
	{
		...
		if(isAnimFinish && scaleX >= 1f)
		{
			...
			if(!kbRectF.isEmpty())
			{
				...
				else if (!kbRectF.contains(curRectF) && !curRectF.contains(kbRectF)) // 都不在范围内
				{
					initKickBackAnim(item, kbRectF.centerX(), kbRectF.centerY(), curRectF.centerX(), curRectF.centerY());
				}
			}
		}
	}
